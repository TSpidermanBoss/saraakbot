...
def filters(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text("Saved filter '`{}`'.".format(keyword), parse_mode=ParseMode.MARKDOWN)
                ...
                try:
                    ...
                    if excp.message == "Unsupported url protocol":
                        message.reply_text("You seem to be trying to use an unsupported url protocol. Telegram "
                                           ...
                                           "again, or ask to @cricket9 for help.")
                    elif excp.message == "Reply message not found":
                        bot.send_message(chat.id, filt.reply, parse_mode=ParseMode.MARKDOWN,
                                         disable_web_page_preview=True,
                                         reply_markup=keyboard)
                    else:
                        message.reply_text("This note could not be sent, as it is incorrectly formatted. Ask to "
                                           "@cricket9 if you can't figure out why!")
